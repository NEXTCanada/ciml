...
that its second derivative is always non-negative.  The second, more
...
the function did not overfit.)  If you replace the zero/one loss with a
...
One of the major conditions for gradient descent being able to find the
...
and a sequence of step sizes $\eta_1, \dots, \eta_K$.  (This is to
...
update says: $\vw \leftarrow \vw - \eta \la \vw = (1- \eta \la) \vw$.  This has
the effect of \emph{shrinking} the weights toward zero.  This is
exactly what we expect the regularizer to be doing!
...
use of some linear algebra.  The key is to set the step size to
...
%
Thus, the derivative is zero for $z\geq1$ and $-1$ for $z<1$, matching
...
valid subderivative at $z=1$.  In fact, the subderivative is always a
...
Thus, $\mat A \vec v$ is well defined if $\mat A$ is $M \times D$, and the resulting product is a vector $\vec u$ with $u_m = \sum_d \mat A_{d,m} v_d$.
  ~\\~\\
  Aside from matrix product, a fundamental matrix operation is inversion. We will often encounter a form like $\mat A \vec x = \vec y$, where $\mat A$ and $\vec y$ are known and we want to solve for $\vec x$.
...
This is precisely the \concept{linear regression} setting.
...
which takes $\cO(D^2 N)$ time.  The inversion takes $\cO(D^3)$ time
